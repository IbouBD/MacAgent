...
class Agent:
    ...
    def step_action(self, key_sequence):
        ...
        for key_combo in key_sequence:
            try:
                time.sleep(0.1)  # Réduction du délai pour fluidité
                ...
                if isinstance(key_combo, str):
                    ...
                    for k in keys:
                        ...
                        if k == '_':
                            key = Key.space  # Remplacement du underscore par espace
                        else:
                            key = self.KEY_MAP.get(k, k)

                            # Normalisation des caractères simples
